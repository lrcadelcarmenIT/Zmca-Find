(() => {
  const API_URL = window.ZMCA_AI_ENDPOINT || "";
  ...
  panel.querySelector("#zmca-ai-form").onsubmit = async (e) => {
    ...
    try {
      if (!API_URL) {
        throw new Error("AI endpoint has not been configured.");
      }
      ...
      addMessage(
        "The AI consultant is not connected yet. Please check the backend configuration.",
        "assistant"
      );
